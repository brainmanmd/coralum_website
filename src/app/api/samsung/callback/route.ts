...
export async function GET(request: Request) {
  ...
  const onboardingUrl = (status: string, reason?: string) => {
    ...
    url.searchParams.set(status === "connected" ? "connected" : "error", "samsung-health");
  ...
  const stateValid = await validateAndClearOAuthState("samsung", state);
  if (!stateValid) {
    return NextResponse.redirect(onboardingUrl("error", "invalid_state"));
  }

  const clientId = process.env.SAMSUNG_CLIENT_ID;
  const clientSecret = process.env.SAMSUNG_CLIENT_SECRET;
  ...
  const tokenResponse = await fetch("https://accounts.samsung.com/account/oauth2/token", {
    method: "POST",
  ...
  await storeTokens("samsung", tokenData.access_token, tokenData.refresh_token);
