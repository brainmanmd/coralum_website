...
export async function GET(request: Request) {
  ...
  const onboardingUrl = (status: string, reason?: string) => {
    ...
    url.searchParams.set(status === "connected" ? "connected" : "error", "oura");
  ...
  const stateValid = await validateAndClearOAuthState("oura", state);
  if (!stateValid) {
  ...
  await storeTokens("oura", tokenData.access_token, tokenData.refresh_token);
